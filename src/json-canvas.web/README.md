...
<script
  type="module"
  src="https://cdn.jsdelivr.net/gh/subtleGradient/web-native@main/src/json-canvas.web/define.js"
></script>
...
- `json-canvas` renders a full JSON Canvas document from:
  ...
  - a fetchable `src` URL.
- `json-canvas` supports native HTML drag-and-drop for moving nodes. Dragging a node updates its `x`/`y`, refreshes connected wires, and emits `json-canvas-node-move`.
