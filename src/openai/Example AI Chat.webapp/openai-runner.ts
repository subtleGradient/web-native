...
const sourcePath = path.join(appRoot, "index.html")
...
function parseChatMessages(source: string): ChatMessage[] {
  ...
  while ((match = messagePattern.exec(source)) !== null) {
    ...
    const text = decodeHtml(pre?.[1] ?? stripTags(match[2])).trim()
    messages.push({
      id: attrs.id ?? attrs["data-id"] ?? attrs["data-source"] ?? `message-${messages.length + 1}`,
      role: attrs["data-role"] ?? "message",
      text,
    })
  }
  return messages
...
function parseFileReferences(source: string): FileReference[] {
  ...
  while ((match = referencePattern.exec(source)) !== null) {
    const attrs = parseAttributes(match[1])
    const filePath = attrs["data-path"]
    if (!filePath) continue
    references.push({
      forId: attrs["data-for"],
