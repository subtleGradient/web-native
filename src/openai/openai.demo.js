...
import "../shadcn/presentational/define.js"
...
function showRunnerWarning() {
  ...
  const localCommand = `bun ./src/openai/scripts/openai-runner.ts ${demoPath}`
...
function currentDemoPath() {
  let pathname = "src/openai/openai.demo.html"
  try {
    pathname = decodeURIComponent(location.pathname).replace(/^\/+/, "")
  } catch {
    return pathname
  }
  const marker = "src/openai/"
  const markerIndex = pathname.lastIndexOf(marker)
  if (markerIndex >= 0) return pathname.slice(markerIndex)
  return pathname.endsWith(".demo.html") ? `src/openai/${pathname.split("/").at(-1)}` : "src/openai/openai.demo.html"
}
