...
```sh
bunx --bun -p https://github.com/subtleGradient/web-native/archive/refs/heads/openai.tar.gz web-native-openai src/openai.webapp/openai.demo.html
```

Or from a local checkout:

```sh
bun ./src/openai.webapp/scripts/openai-runner.ts src/openai.webapp/openai.demo.html
```

To open one small demo directly, replace the path with a focused page such as `src/openai.webapp/openai.web-search.demo.html`.

Responses WebSocket Mode demo with sandboxed local `eval_js` tool:

```sh
bun ./src/openai.webapp/scripts/openai-runner.ts src/openai.webapp/openai.responses-websocket-eval.demo.html
```
