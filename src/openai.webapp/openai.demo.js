...
function showRunnerWarning() {
  ...
  const localCommand = `bun ./src/openai.webapp/scripts/openai-runner.ts ${demoPath}`
...
function currentDemoPath() {
  let pathname = "src/openai.webapp/openai.demo.html"
  try {
    pathname = decodeURIComponent(location.pathname).replace(/^\/+/, "")
  } catch {
    return pathname
  }
  const marker = "src/openai.webapp/"
  const markerIndex = pathname.lastIndexOf(marker)
  if (markerIndex >= 0) return pathname.slice(markerIndex)
  return pathname.endsWith(".demo.html") ? `src/openai.webapp/${pathname.split("/").at(-1)}` : "src/openai.webapp/openai.demo.html"
}
